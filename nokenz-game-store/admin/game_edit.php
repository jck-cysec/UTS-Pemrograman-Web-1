    ...
    <div class="row">
        ...
        <div class="col-md-2 bg-dark text-white p-4" style="min-height:100vh;">
            ...
            <ul class="nav flex-column">
                ...
                <li class="nav-item mb-2">
                    <a href="game_list.php" class="nav-link text-white active">
                        <i class="bi bi-controller"></i> Manage Games
                    </a>
                </li>
                <li class="nav-item mb-2">
                    <a href="category_manage.php" class="nav-link text-white">
                        <i class="bi bi-tags"></i> Manage Categories
                    </a>
                </li>
                <li class="nav-item mb-2">
                    <a href="users_list.php" class="nav-link text-white">
                        <i class="bi bi-people"></i> Manage Users
                    </a>
                </li>
                <li class="nav-item mb-2">
                    <a href="orders_list.php" class="nav-link text-white">
                        <i class="bi bi-box-seam"></i> Manage Orders
                    </a>
                </li>
                <li class="nav-item mb-2">
                    <a href="news_list.php" class="nav-link text-white">
        ...
        <div class="col-md-10 p-4">
            ...
            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white">
                    <h5 class="mb-0"><i class="bi bi-joystick"></i> Edit Game Information</h5>
                </div>
                <div class="card-body">
                    ...
                    <form id="editGameForm">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="nama_game" value="<?= htmlspecialchars($game['nama_game']) ?>" required>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" name="harga" value="<?= $game['harga'] ?>" required>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold">Description *</label>
                            <textarea class="form-control" name="deskripsi" rows="4" required><?= htmlspecialchars($game['deskripsi']) ?></textarea>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold">Image Filename *</label>
                            <input type="text" class="form-control" name="gambar" value="<?= htmlspecialchars($game['gambar']) ?>" required>
                        </div>
...
<script>
// LOAD GENRES & SELECT CURRENT VALUE
function loadGenres(selected) {
    ...
    .then(list => {
        ...
        list.forEach(g => {
            sel.innerHTML += `<option value="${g}" ${g === selected ? "selected" : ""}>${g}</option>`;
        });
    });
}

// LOAD PLATFORMS & SELECT CURRENT VALUE
function loadPlatforms(selected) {
    ...
    .then(list => {
        ...
        list.forEach(p => {
            sel.innerHTML += `<option value="${p}" ${p === selected ? "selected" : ""}>${p}</option>`;
        });
    });
}

document.addEventListener("DOMContentLoaded", () => {
    loadGenres("<?= $game['genre'] ?>");
    loadPlatforms("<?= $game['platform'] ?>");
});
...
document.getElementById("editGameForm").addEventListener("submit", e => {
    ...
    fetch("../api/api.php?action=update_game", {
        method: "POST",
        body: fd
    })
    .then(r => r.json())
    .then(res => {
        if (res.status) {
            ...
            setTimeout(() => location.href = "game_list.php", 1200);
        } else {
